...
Authorization: Bearer <token>
```
Returns real-time monitoring data including active jobs, performance metrics, system health, and alerts. Requires one of: `OPERATIONS_MANAGER`, `ADMIN`, `JOB_VIEWER`, or `JOB_EXECUTOR` role.
...
- **E2E**: 29 Playwright specs covering auth, dashboard, configuration, monitoring, templates, sidebar, and regression scenarios
...
The platform includes a real-time monitoring dashboard accessible at `/monitoring` in the UI. It provides:

- **Active Jobs Grid**: Live view of running batch jobs with progress, throughput, and error counts
- **Performance Metrics**: CPU usage, memory usage, success/error rates, throughput
- **System Health**: Database connectivity, JVM memory, batch processing subsystem status
- **Alerts Panel**: Auto-generated alerts from failed or long-running jobs with acknowledge capability
- **Historical Trends**: 24-hour hourly aggregation of executions, throughput, and error rates

**API Endpoint:**
```bash
# Fetch dashboard data (requires OPERATIONS_MANAGER, ADMIN, JOB_VIEWER, or JOB_EXECUTOR role)
curl -H "Authorization: Bearer <token>" http://localhost:8080/api/monitoring/dashboard
```

The frontend polls this endpoint every 15 seconds for near-real-time updates.
...
**Status**: US001 Complete + Phase 3 Manual Batch Execution Complete + Monitoring Dashboard - Full End-to-End Implementation
