...
public class FieldTemplateRepository {
    ...
    public List<FieldTemplateEntity> findByFileTypeAndTransactionTypeAndEnabledOrderByTargetPosition(String fileType, String transactionType, String enabled) {
        try {
            ...
            List<FieldTemplateEntity> result = jdbcTemplate.query(sql, createBasicRowMapper(), fileType, transactionType, enabled);
            log.info("✅ FIXED: Query executed successfully for {}/{}/{}, found {} results", fileType, transactionType, enabled, result.size());
            
            // If no results found with enabled filter, try without filter to see what's available
            if (result.isEmpty()) {
                ...
                List<FieldTemplateEntity> allResults = jdbcTemplate.query(debugSql, createBasicRowMapper(), fileType, transactionType);
                log.info("🔍 DEBUG: Found {} total records for {}/{} (any enabled status)", allResults.size(), fileType, transactionType);
            ...
            log.error("Error querying field_templates table for {}/{}/{}: {}", fileType, transactionType, enabled, e.getMessage());
    ...
    public List<String> findTransactionTypesByFileType(String fileType) {
        try {
            ...
            log.error("Error querying transaction types from field_templates for fileType: {}: {}", fileType, e.getMessage());
    ...
    public Optional<FieldTemplateEntity> findByFileTypeAndTransactionTypeAndFieldName(String fileType, String transactionType, String fieldName) {
        try {
            String sql = "SELECT FILE_TYPE, TRANSACTION_TYPE, FIELD_NAME, TARGET_POSITION, LENGTH, " +
                        "DATA_TYPE, FORMAT, REQUIRED, DESCRIPTION, ENABLED, CREATED_BY, CREATED_DATE, " +
                        "MODIFIED_BY, MODIFIED_DATE " +
                        "FROM cm3int.field_templates " +
                        "WHERE FILE_TYPE = ? AND TRANSACTION_TYPE = ? AND FIELD_NAME = ?";
            List<FieldTemplateEntity> results = jdbcTemplate.query(sql, createBasicRowMapper(), fileType, transactionType, fieldName);
            return results.isEmpty() ? Optional.empty() : Optional.of(results.get(0));
        } catch (Exception e) {
    ...
    public Optional<FieldTemplateEntity> findByFileTypeAndTransactionTypeAndTargetPosition(String fileType, String transactionType, Integer targetPosition) {
        try {
            String sql = "SELECT FILE_TYPE, TRANSACTION_TYPE, FIELD_NAME, TARGET_POSITION, LENGTH, " +
                        "DATA_TYPE, FORMAT, REQUIRED, DESCRIPTION, ENABLED, CREATED_BY, CREATED_DATE, " +
                        "MODIFIED_BY, MODIFIED_DATE " +
            ...
            List<FieldTemplateEntity> results = jdbcTemplate.query(sql, createBasicRowMapper(), fileType, transactionType, targetPosition);
            return results.isEmpty() ? Optional.empty() : Optional.of(results.get(0));
        } catch (Exception e) {
    ...
    public void deleteByFileTypeAndTransactionTypeAndFieldName(String fileType, String transactionType, String fieldName) {
