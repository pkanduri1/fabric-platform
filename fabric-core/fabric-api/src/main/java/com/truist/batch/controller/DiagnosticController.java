...
public class DiagnosticController {
    ...
    public ResponseEntity<Map<String, Object>> getOracleStatus() {
        ...
        try {
            ...
            Integer manualJobConfigExists = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM USER_TABLES WHERE TABLE_NAME = 'MANUAL_JOB_CONFIG'", Integer.class);
            ...
            Integer fieldTemplatesExists = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM USER_TABLES WHERE TABLE_NAME = 'FIELD_TEMPLATES'", Integer.class);
            status.put("field_templates_exists", fieldTemplatesExists > 0);
            
            // Count existing configurations
            if (manualJobConfigExists > 0) {
                Integer configCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM MANUAL_JOB_CONFIG", Integer.class);
                ...
                status.put("recent_configs", recentConfigs.size() > 5 ? recentConfigs.subList(0, 5) : recentConfigs);
            }
            
            // Count existing field templates
            if (fieldTemplatesExists > 0) {
                Integer templateCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM FIELD_TEMPLATES", Integer.class);
    ...
    public ResponseEntity<Map<String, Object>> testTemplateToConfigSave(
        ...
        try {
            ...
            // Step 1: Create configuration from template (this is what the TemplateController does)
            ...
            result.put("field_mappings_count", config.getFieldMappings() != null ? config.getFieldMappings().size() : 0);
            ...
            result.put("step3_verification", verificationCount > 0 ? "FOUND_IN_ORACLE" : "NOT_FOUND_IN_ORACLE");
            ...
            result.put("message", "Configuration successfully created from template and saved to Oracle database");
    ...
    public ResponseEntity<Map<String, Object>> getBatchConfigurations() {
        ...
        try {
            ...
            Integer testCount = jdbcTemplate.queryForObject(
                ...
                "WHERE SOURCE_SYSTEM = 'encore' AND JOB_NAME LIKE '%test%'", Integer.class);
